...
<div class="space-y-6">
	<div class="flex items-center justify-between">
		<h1 class="text-2xl font-bold text-slate-800">Perfil del cliente</h1>
		<a href="/clientes" class="text-indigo-600 hover:underline">← Volver al listado</a>
	</div>

	<div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
		<div class="bg-white rounded-lg shadow p-6 lg:col-span-2">
			<h2 class="text-lg font-semibold text-slate-800 mb-4">Datos generales</h2>
			<div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
				<div>
					<p class="text-slate-500">Nombre</p>
					<p class="font-medium text-slate-800">{cliente.nombre}</p>
				</div>
				<div>
					<p class="text-slate-500">Empresa</p>
					<p class="font-medium text-slate-800">{cliente.empresa || '-'}</p>
				</div>
				<div>
					<p class="text-slate-500">RFC</p>
					<p class="font-medium text-slate-800">{cliente.rfc || '-'}</p>
				</div>
				<div>
					<p class="text-slate-500">Correo</p>
					<p class="font-medium text-slate-800">{cliente.correo}</p>
				</div>
				<div>
					<p class="text-slate-500">Teléfono</p>
					<p class="font-medium text-slate-800">{cliente.telefono || '-'}</p>
				</div>
				<div>
					<p class="text-slate-500">Dirección</p>
					<p class="font-medium text-slate-800">{cliente.direccion || '-'}</p>
				</div>
				<div class="md:col-span-2">
					<p class="text-slate-500">Notas</p>
					<p class="font-medium text-slate-800">{cliente.notas || '-'}</p>
				</div>
			</div>
		...
		<div class="space-y-4">
			<div class="bg-white rounded-lg shadow p-6">
				<p class="text-slate-500 text-sm">Total facturado</p>
				<p class="text-2xl font-bold text-indigo-700">{formatoMoneda(data.totalFacturado)}</p>
			</div>
			<div class="bg-white rounded-lg shadow p-6">
				<p class="text-slate-500 text-sm">Total cobrado</p>
	...
	<div class="bg-white rounded-lg shadow overflow-hidden">
		<h2 class="text-lg font-semibold text-slate-800 p-6 pb-0">Cotizaciones</h2>
			...
			<tbody class="divide-y divide-slate-100">
				{#each cliente.cotizaciones as cot}
					<tr class="hover:bg-slate-50">
						...
						<td class="px-6 py-4 text-right text-slate-800">{formatoMoneda(Number(cot.total))}</td>
					</tr>
				{:else}
					<tr>
